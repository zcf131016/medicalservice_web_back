...
public class CaseController {
    ...
    @RequestMapping("/findCasesByPage/{pageNum}/{pageSize}")
    public Result findCasesByPage(@PathVariable Integer pageNum, @PathVariable Integer pageSize){
        ...
        return Result.success().setData(casesList).setCode(ResultCodeEnum.OK.getCode()).setMsg("分页查找案例成功");
    }
    @ApiOperation(value = "根据courseId查找所有案例")
    @ApiParam(name="courseId",type = "Integer")
    @RequestMapping("/findCasesbycourseId/{courseId}")
    ...
    @RequestMapping("/findCasesbycaseName/{caseName}")
    ...
    @RequestMapping("/findCasesbyteacherId/{creatTeacher}")
    ...
    @RequestMapping("/findCasesbycaseId/{caseId}")
    ...
    @RequestMapping("/batchdeletecase")
